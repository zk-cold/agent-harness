...
4. The primary agent must not rewrite raw tool outputs (unmodified direct tool output) or raw data into a narrative briefing for the critic sub-agent. For prompt-only critic sub-agents, raw tool outputs provided in the prompt must be unmodified direct artifacts (coverage reports, test output, diff output). For tool-capable critic sub-agents, the primary agent provides no raw tool outputs in the prompt — the critic sub-agent fetches what it needs.
