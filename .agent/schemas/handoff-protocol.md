...
- **Session-start check.** On session start, the primary agent must follow `/new-sdlc` bootstrap against the applicable target repo before any interview or execution work begins. If `handoff.md` or `mission.md` exists at that target repo root, the primary agent must stop and ask the dev how to proceed before continuing. During bootstrap, the primary agent must inspect the target repo's `.claude/worktrees/` directory and surface any related `handoff.md` to the dev before proceeding. Treat a worktree handoff as related when the current request and the handoff share the same target repo and at least one of the following is true from the visible artifacts: they name the same deliverable, they touch the same code area or file set, or the current request appears to continue or replace the same blocker or acceptance criteria. If the available artifacts do not let the primary agent rule relatedness out confidently, treat the handoff as related. If the primary agent will treat a related handoff as fresh work instead of resuming it, the primary agent must confirm with the dev before discarding or replacing it. The primary agent must create a worktree in the target repo before any execution work begins. Whenever the primary agent later spawns a sub-agent, instantiate the matching prompt from `.agent/templates/new-sdlc-subagents.prompt.template`, with `.agent/schemas/governance-schema.md` as the first governance artifact listed. Spawned sub-agents resume only from the role-specific artifacts or worktree context explicitly handed to them and must not perform their own bootstrap discovery unless the active phase explicitly requires it.
...
## Abort Protocol Reference
This document relies on the existing abort protocol behavior: continuing the current mission may require reopening approved work, and an aborted mission records `Aborted: approved mission can no longer continue because <blocker summary>. This mission is already aborted and must not be resumed. Await a fresh user request.`
