...
7. Recovery reuses the existing worktree. Before recovery continues, discard the unstaged changes that affect governance files. For this rule, governance files are `AGENTS.md`, `CLAUDE.md`, files under `.agent/schemas/`, files under `.claude/commands/`, and any `mission.md` changes inside its `Invariants` or `Beliefs` sections. If a file contains both governance and non-governance unstaged changes, discard only the governance portion and preserve the non-governance changes. Unstaged changes to non-governance code files may remain in place.
