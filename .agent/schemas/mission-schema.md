...
A mission.md file defines a single unit of work for the agent harness. It is generated by the lead agent during the dev interview phase at the worktree root and reviewed by critic agents before execution is permitted. Every mission.md must be self-contained: a critic with access to only `CLAUDE.md`, `.agent/schemas/critic-protocol.md`, and `mission.md` must be able to evaluate it.
