...
The lead agent spawns a dev subagent and hands it the approved `mission.md`. Instantiate the `Dev Agent Execution` template from `.agent/templates/new-sdlc-subagents.prompt.template`. The exact `mission.md` approved in the preceding review phase is the execution contract and must remain unchanged. The dev agent must produce only the deliverables specified in scope and must leave implementation changes uncommitted in the worktree for post-implementation review. If implementation reveals that the mission itself must change, the dev agent must stop and report the blocker to the lead agent, who then follows `.agent/schemas/abort-protocol.md`.
