...
## API Design
API designs must be explicit as invariants or external constraints, such that tests can be written without knowing the implementations. When a mission introduces or changes an API, the lead writes the minimum prod-scope scaffolding (signatures, type declarations, module skeletons) before handing off to the SDET; the SDE then fills in behavior under TDD.
