...
Spawn one critic agent. The critic performs two checks in order. The critic reads only the artifacts named in Inputs plus the allowed raw eligibility tool outputs and must follow the listed critic protocol. If the critic cannot be spawned, follow Required Critic Availability before taking any other action. The critic prompt must contain only the artifact references listed in Inputs plus raw eligibility tool outputs, with no lead-agent context briefing, summary, hidden rationale, "things to keep in mind," or extra "mentions." If anything needs to be included before approval, put it in `mission.md` or another phase-allowed artifact first, then rerun review.
