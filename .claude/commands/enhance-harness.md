...
Draft a `mission.md` at the worktree root following the repo-root `.agent/schemas/mission-schema.md`. The mission must be self-contained: a critic with access to only the repo-root `CLAUDE.md`, the repo-root `.agent/schemas/critic-protocol.md`, and `mission.md` must be able to evaluate it.
